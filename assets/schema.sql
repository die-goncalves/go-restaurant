...
create table if not exists operating_hours (
  ...
  constraint operating_hours_store_id_fkey foreign key (store_id) references stores (id) on delete cascade
);
...
create table if not exists orders (
  ...
  constraint orders_user_fkey foreign key (user_id) references auth.users (id)
);
...
create table if not exists product_ratings (
  ...
  constraint product_ratings_order_product_id_key unique (order_product_id),
  constraint product_ratings_order_product_fkey   foreign key (order_product_id) references order_products (id),
  constraint product_ratings_product_fkey         foreign key (product_id)       references products       (id) on delete cascade,
  constraint product_ratings_store_id_fkey        foreign key (store_id)         references stores         (id),
  constraint product_ratings_user_fkey            foreign key (user_id)          references auth.users     (id)
);
...
-- begin
--   -- Verifica se o order_product_id pertence a um pedido do mesmo user_id
--   if not exists (
--     select 1
...
-- or
-- update on product_ratings for each row
